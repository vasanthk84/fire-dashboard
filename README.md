...
### Kite wheel cash capital
FIRE defaults to Kite opening cash balance via OI Analyzer /api/wheel-journal/external/funds. Set server-only OI_ANALYZER_TOKEN in FIRE to match FIRE_DASHBOARD_TOKEN on OI Analyzer, and OI_ANALYZER_URL to its reachable backend. Deploy OI first. This assumes all opening cash is dedicated to wheeling, excluding shares and intraday cash movements. Available and used margin are separate. Calculations and the yield chart use the effective capital; uncheck the Kite option for manual mode. Failures or missing configuration visibly fall back to saved manual capital. Refresh capital reruns the calculation.
